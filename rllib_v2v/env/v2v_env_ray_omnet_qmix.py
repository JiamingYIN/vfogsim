...
class V2V_Env_ray_OMNET_qmix(MultiAgentEnv):
    """
    A RLLIB environment for testing MARL environments with SUMO simulations.
    """
    def __init__(self, config):
        """Initialize the environment."""
        super(V2V_Env_ray_OMNET_qmix, self).__init__()
        print(config)
        self._config = config
        ...
        # Spaces
        self.observation_space = Dict({agent: self.get_obs_space(agent) for agent in self._agent_ids})
            ...
            for f in os.listdir(dir):
                if os.path.isfile(os.path.join(dir, f)):
                    try:
                        ...
                        print("Fail to remove {}".format(os.path.join(dir, f)))
    ...
    def get_observation(self, edge_states, agent_id):
        ...
        env_obs = [min(1.0, o / 100) for o in env_obs]
        obs = dict({"obs": np.array(env_obs + mask)})
        #obs = dict()
    ...
    def get_obs_space(self, agent):
        return gym.spaces.Dict({"obs": gym.spaces.Box(0.0, 1.0, shape=(30,), dtype=np.float32)})

...
from env import VFN_2hop


class V2V_Env_ray_OMNET_2hop(MultiAgentEnv):
    """
    A RLLIB environment for testing MARL environments with SUMO simulations.
    """
    def __init__(self, config):
        """Initialize the environment."""
        super(V2V_Env_ray_OMNET_2hop, self).__init__()
        print(config)
        self._config = config
        ...
        self.data['action_map'] = self.data['action_map'][['straight', 'left', 'right', 'Uturn']]
        print(self.data['action_map'].head(3))
        ...
        for agent, agent_config in self._config["agent_init"].items():
            self.agents[agent_config['vfn_id']] = VFN_2hop(agent_config)
    ...
    def get_observation(self, edge_states, agent_id):
        ...
        print("One hop action roads: ", action_roads)
        env_obs = []
        mask = []
        for road in action_roads:
            if road < 0:
                env_obs.extend([0.0] * 5 * 5)  # one-hop neighbor + 4 2-hop neighbors
                mask.extend([0.0] * 5)
            else:
                act_road = self.data['road_id_map'][road]
                env_obs.extend(edge_states[act_road])
                mask.append(1.0)  # 0.0: remove one-hop neighbor, 1.0: include one-hop neighbor
                two_hop_neighbors = self.data['action_map'].loc[act_road].values

                '''
                if two_hop_neighbors.sum() == -4:
                    mask.append(1.0)
                else:
                    mask.append(0.0)
                '''
                for nbr_road in two_hop_neighbors:
                    if nbr_road < 0:
                        env_obs.extend([0.0] * 5)
                        mask.append(0.0)
                    else:
                        act_nbr_road = self.data['road_id_map'][nbr_road]
                        env_obs.extend(edge_states[act_nbr_road])
                        mask.append(1.0)
        print(mask)
        mask[-1] = 0.0  # Mask Uturn + Uturn
        obs = dict()
    ...
    def step(self, action_dict):
        ...
        for agent_id in shuffled_agents:
            ...
            if self.agents[agent_id].time_to_act:
                ...
                if action_dict[agent_id] % 5 == 0:  # one-hop neighbors
                    action_road_ind = road_action_map[action_dict[agent_id] // 5]
                else:
                    one_hop_act_ind = action_dict[agent_id] // 5
                    two_hop_act_ind = action_dict[agent_id] % 5 - 1
                    action_road_ind = self.data['action_map'].iloc[road_action_map[one_hop_act_ind]].values[two_hop_act_ind]
                #print("Take action (Agent ", agent_id, "):", road_id, action_road_ind, action_dict[agent_id])
                      #self.data['road_id_map'][action_road_ind])
                if action_dict[agent_id] == 15:  # U-turn
    ...
    def get_env_obs_space(self, agent):
        ...
        # TODO: Range
        return gym.spaces.Box(0.0, 100.0, shape=(100,), dtype=np.float32)

    def get_obs_space(self, agent):
        return gym.spaces.Dict(
            {"obs": gym.spaces.Box(0.0, 100.0, shape=(100,), dtype=np.float32),
             "mask": gym.spaces.Box(0, 1, shape=(20,), dtype=np.float32)}
        )
    def get_mask_space(self, agent):
        return gym.spaces.Box(0, 1, shape=(20,), dtype=np.float32)
